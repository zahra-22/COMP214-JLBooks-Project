...
//     res.status(500).json({ message: "Error registering author.", error: err.message });
//   } finally {
...
//   if (!isbn || !authorId)
//     return res.status(400).json({ message: "ISBN and Author ID are required" });
...
//     res.json({ success: true, message: "Author assigned to book successfully." });

//   } catch (err) {
//     console.error("Assign Author Error:", err);
//     res.status(500).json({ message: "Error assigning author", error: err.message });
//   } finally {
//     if (conn) await conn.close();
//   }
// });
